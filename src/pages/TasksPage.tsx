...
import { Plus, Calendar, Clock, Timer, AlertTriangle, CheckCircle2, Filter } from 'lucide-react';
import { api, type Task } from '@/lib/api';
import { format, isToday } from 'date-fns';
import { calculateTimeRemaining, formatTimeRemaining, getTimeUrgencyColor, getUrgencyBadgeVariant } from '@/utils/timeUtils';
...
export function TasksPage() {
  ...
  const overdueTasks = sortedPendingTasks.filter(task => calculateTimeRemaining(task.deadline).isOverdue);
  ...
  const TaskTimerCard = ({ task }: { task: Task }) => {
    ...
    return (
      <Card className={`transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${
        timeRemaining.isOverdue ? 'border-red-500 bg-red-50/50 dark:bg-red-950/20' :
        timeRemaining.totalMinutes <= 60 ? 'border-orange-500 bg-orange-50/50 dark:bg-orange-950/20' :
        'hover:bg-accent/50'
      }`}>
        ...
        <CardContent className="pt-0">
          ...
          <div className="text-center mb-4">
            <div className={`text-4xl font-mono font-bold ${urgencyColor} mb-1`}>
          ...
          <div className="space-y-2">
            <h3 className="font-bold text-center text-lg leading-tight">
              <span className="font-black">{task.title.charAt(0)}</span>{task.title.slice(1)}
            </h3>
